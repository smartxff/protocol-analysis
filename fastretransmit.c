...
int main(int argc, char **argv)
{
	...
	while(1){
		...
		if(seq == double_seq && retrans_count >0)  //开始重传后，收到之前保存的seq时，退出重传
		{
			enter_retrans = 2;
		}
		...
		if ((flag  == 0x10 && enter_retrans == 0) || enter_retrans == 1){
			//recvcount 小于等于2时，不发送任何确认包。
			if(recvcount > 2)   
			{
				my_seq++;
				printf("----------------------------------------------send fastretrans\n");
				last_seq = ack_seq;
				retrans_count++;
				int mesg_len_a = make_message(send_message, MAXLINE, argv, double_seq,fg);
		...
		if(recvcount == 2){          //当收到2个延迟确认的包时触发快速重传
			enter_retrans = 1;
			double_seq = ack_seq;      //保存下一个数据包的seq，等再次收到此seq的数据包时，关闭数据包重传
		}
